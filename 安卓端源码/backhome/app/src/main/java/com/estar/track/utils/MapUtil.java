...
import static com.estar.track.utils.BitmapUtil.bmArrowPoint;
import static com.estar.track.utils.BitmapUtil.bmEnd;
import static com.estar.track.utils.BitmapUtil.bmStart;
...
public class MapUtil {
     ...
     * 绘制历史轨迹并且重新添加范围圆圈
     */
    public void drawHistoryTrack(List<LatLng> points, SortType sortType,BaiduMap map,LatLng nowLatLng,int i) {
        // 绘制新覆盖物前，清空之前的覆盖物
        baiduMap.clear();
        if (points == null || points.size() == 0) {
            if (null != polylineOverlay) {
        ...
        TracingActivity.drawActionCircle(nowLatLng,1400,map);
    ...
    public void drawAnotherMemberHistoryTrack(List<LatLng> points, SortType sortType,BaiduMap map,LatLng nowLatLng,int i) {
        // 绘制新覆盖物前，清空之前的覆盖物
        if (points == null || points.size() == 0) {
            if (null != polylineOverlay) {
                polylineOverlay.remove();
